...
func TestRunPromptImprove(t *testing.T) {
	env := testutil.SetupTestEnv(t)
	env.SetEnv("NOTTE_API_KEY", "test-key")
	...
	server.AddResponse("/prompts/improve", 200, `{"prompt":"improved","variables":[]}`)

	origPrompt := promptText
	promptText = "improve me"
	t.Cleanup(func() { promptText = origPrompt })
...
func TestRunPromptNudge(t *testing.T) {
	env := testutil.SetupTestEnv(t)
	env.SetEnv("NOTTE_API_KEY", "test-key")
	...
	server.AddResponse("/prompts/nudge", 200, `{"prompt":"nudged"}`)
